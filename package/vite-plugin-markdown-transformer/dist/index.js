...
function MarkdownTransformer(option = {
  ...
  const regText = `<${option.matchTag || defaultMatchTag}>(?<content>[\\s\\S]*?)</markdown>`;
  const markdownTagReg = new RegExp(regText, "g");
  const markdownContent = new RegExp(regText);
  const vueFileRegex = /\.vue$/;
  const md = new import_markdown_it.default(option.markdownItOptions || {});
  option.plugins && option.plugins.forEach((r) => {
